...
public final class SkinManagerClient {
    ...
    public static Identifier getOrFetch(AbstractClientPlayer player) {
        if (player == null) {
            return null;
        }
        ...
        selected.thenCompose(skin -> {
            ...
            client.execute(() -> {
                ...
                try {
                    ...
                    DynamicTexture baseTexture = new DynamicTexture(() -> "catskinc_base_skin_" + uuid, images.skinImage);
                    skinConsumed = true;
                    ...
                    if (talkingImage != null) {
                        DynamicTexture talkingTexture = new DynamicTexture(() -> "catskinc_talking_skin_" + uuid, talkingImage);
        ...
        try {
            ...
            for (int y = 0; y < targetHeight; y++) {
                ...
                for (int x = 0; x < targetWidth; x++) {
                    ...
                    int overlayColor = overlayImage.getPixel(ox, oy);
                    int alpha = (overlayColor >>> 24) & 0xFF;
                    merged.setPixel(x, y, alpha > 0 ? overlayColor : skinImage.getPixel(sx, sy));
                }
            }
            return merged;
        } catch (Exception exception) {
